...
- [ ] Abstract `Account` (or interface + abstract base): account number, owner, balance (private!), `deposit`, `withdraw`, transaction history
- [ ] `CheckingAccount` allows overdraft down to a per-account limit (e.g., −500)
...
- [ ] Custom exceptions, each carrying relevant data as fields with getters:
  ...
  - [ ] Invalid amounts (zero, negative, NaN) throw `IllegalArgumentException` (unchecked) — a comment explains *why* this one is unchecked while the others are checked
...
- Currency-safe money: replace `double` with `long` cents everywhere and note every place the change caught a rounding bug.
