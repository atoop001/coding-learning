...
Alongside the render tree, the browser builds a parallel structure it doesn't paint at all: the **accessibility tree**. It's what screen readers, switch devices, and other assistive tech actually consume — a tree of *roles*, *names*, and *states* ("button, 'Submit', enabled") derived from the DOM. **Semantic HTML** (`<button>`, `<nav>`, `<a href>`, `<input type="checkbox">`) populates this tree correctly for free, because each tag carries a built-in role and keyboard behavior. A `<div onclick="...">` styled to look like a button gets none of that — visually identical, but invisible to the accessibility tree until you patch it with **ARIA** attributes (`role="button"`, `tabindex="0"`, `aria-pressed`, plus JS to handle keyboard events browsers give `<button>` automatically). The rule of thumb you'll hear everywhere: "no ARIA is better than bad ARIA" — reach for the real semantic element first; ARIA is a patch for when semantics genuinely can't express what you need.
...
6. **Accessibility tree comparison.** Build (or find) a page with a real `<button>` and a `<div>` styled identically but wired up with `onclick` instead. Open DevTools → **Accessibility** panel (or the "Accessibility" pane in Elements), select each element, and compare their entries in the accessibility tree — role, name, and whether it's keyboard-focusable (try Tab). Write two sentences on what the `<div>` is missing and which ARIA attributes plus JS would be needed to close the gap.
