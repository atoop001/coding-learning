...
6. `members.email` is already `UNIQUE`, which makes it a natural conflict target for **UPSERT**. Write one `INSERT ... ON CONFLICT (email) DO UPDATE` statement that adds a member row for a given `(name, email, plan, signup_date, age)` if the email is new, or updates that member's `plan` (and only `plan`) if the email already exists — use `excluded.plan` to reference the incoming value. Run it twice with the same email and different plans, and confirm the second run updated rather than duplicated the row.
