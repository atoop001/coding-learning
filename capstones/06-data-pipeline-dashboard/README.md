...
- **SQLite-in-repo vs. hosted Postgres — an honest tradeoff.** Two legitimate storage layers:
  ...
  - *Hosted Postgres (several providers have free tiers):* the dashboard can query it live, no commit-the-database weirdness, and it's closer to how production systems actually work — but you manage credentials as secrets, and free tiers can idle, expire, or change terms mid-project.
